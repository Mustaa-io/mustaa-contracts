...
import {ILSP7DigitalAssetTime} from "./ILSP7DigitalAssetTime.sol";
 ...
 *
 * @dev This contract implement the core logic of the functions for the {ILSP7DigitalAssetTime} interface.
 ...
 */
abstract contract LSP7DigitalAssetInitAbstractTime is
    ILSP7DigitalAssetTime,
    LSP4DigitalAssetMetadataInitAbstract,
    LSP17Extendable
{
    ...
    // Mapping from `tokenOwner` to an `amount` of tokens per year
    mapping(uint256 => mapping(address => uint256)) internal _tokenOwnerYearlyBalances;

    // Mapping from `year` to an `amount` of tokens (total supply per year)
    ...
    // Mapping a `tokenOwner` to an `operator` to `year` to `amount` of tokens.
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        internal _operatorYearlyAuthorizedAmount;
    ...
    /**
     * @inheritdoc ILSP7DigitalAssetTime
     */
    ...
    function _update(
        ...
        // Handle total supply (_existingTokens) exactly like the original
        if (from == address(0)) {
